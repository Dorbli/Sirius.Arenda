...
@router.post("/bookings", status_code=status.HTTP_201_CREATED, summary="Создание брони",
             responses={
                 status.HTTP_404_NOT_FOUND: {"description": "Комната не найдена"},
                 status.HTTP_400_BAD_REQUEST: {"description": "Неверный формат даты"},
                 status.HTTP_409_CONFLICT: {"description": "Пространство уже занято"}
             })
...
@router.delete("/bookings/{booking_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Отмена брони",
               responses={
                   status.HTTP_404_NOT_FOUND: {"description": "Бронь не найдена, либо пользователь её не бронировал"}
                   })
...
@router.get("/rooms/{room_id}/bookings", summary="Получить все брони по дате и комнате", responses={
    status.HTTP_400_BAD_REQUEST: {"description": "Неверный формат даты"},
    status.HTTP_404_NOT_FOUND: {"description": "Броней нет"}
})
...
@router.get("/bookings", summary="Получить все брони")
def get_all_books():
